...
func _update_scatter() -> void:
	target_position = scatter_target
	...
	if player:
		...
		match ghost_type:
			...
			GhostType.PINKY:   # 4 tiles ahead of player
				...
				target_position = player.global_position + player_dir * 4 * GRID_SIZE
			GhostType.INKY:    # 2x vector from Blinky to 2 tiles ahead of player
				if player:
					...
					if blinky:
						...
						target_position = player_ahead + vector_to_blinky * 2
					else:
						target_position = player.global_position  # Fallback
				else:
			...
			GhostType.CLYDE:   # Chase if far, scatter if close
				if player:
					...
					if distance_to_player > 8 * GRID_SIZE:  # More than 8 tiles away
						...
						target_position = scatter_target  # Go to scatter corner
				else:
					target_position = scatter_target
...
func _update_eyes_returning() -> void:
	...
	target_position = ghost_house_center
...
func _update_spawning() -> void:
	...
	if global_position.distance_to(ghost_house_center) < GRID_SIZE:
		state = State.SCATTER  # Start in scatter state after spawning
...
func can_move(dir: Vector2) -> bool:
	...
	query.collision_mask = 1  # Assuming walls are on layer 1 (adjust as needed)
