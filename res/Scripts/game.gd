...
# These will be set in _position_entities
var pacman: CharacterBody2D
var blinky: CharacterBody2D
var pinky: CharacterBody2D
var inky: CharacterBody2D
var clyde: CharacterBody2D
...
func _position_entities() -> void:
	# Instance Pac-Man
	var pacman_scene = preload("res://Scenes/Pacman.tscn")
	pacman = pacman_scene.instantiate()
	pacman.global_position = Vector2(13.5 * 16, 17 * 16)  # Example position, adjust as needed
	add_child(pacman)
	
	# Instance ghosts
	var ghost_scene = preload("res://Scenes/Ghost.tscn")
	var ghost_positions = [
		Vector2(13.5 * 16, 11 * 16),  # Blinky
		Vector2(13.5 * 16, 11 * 16),  # Pinky
		Vector2(13.5 * 16, 11 * 16),  # Inky
		Vector2(13.5 * 16, 11 * 16)   # Clyde
	]
	var ghost_types = [
		Ghost.GhostType.BLINKY,
		Ghost.GhostType.PINKY,
		Ghost.GhostType.INKY,
		Ghost.GhostType.CLYDE
	]
	
	for i in 4:
		var ghost_instance = ghost_scene.instantiate()
		ghost_instance.global_position = ghost_positions[i]
		# Set the ghost type via the exported variable
		ghost_instance.ghost_type = ghost_types[i]
		add_child(ghost_instance)
		ghosts.append(ghost_instance)
	
	# Assign individual ghost references (assuming order: Blinky, Pinky, Inky, Clyde)
	if ghosts.size() >= 4:
		blinky = ghosts[0]
		pinky = ghosts[1]
		inky = ghosts[2]
...
func _for_each_ghost(callback) -> void:
	for ghost in ghosts:
		if ghost:
			callback(ghost)
