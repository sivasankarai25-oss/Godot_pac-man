...
func _ready() -> void:
	# Set up audio buses
	AudioServer.set_bus_volume(AudioServer.get_bus_index(MASTER_BUS), master_volume)
	AudioServer.set_bus_volume(AudioServer.get_bus_index(MUSIC_BUS), music_volume)
	AudioServer.set_bus_volume(AudioServer.get_bus_index(SFX_BUS), sfx_volume)
...
func mute_master(muted: bool) -> void:
	...
	AudioServer.set_bus_volume(AudioServer.get_bus_index(MASTER_BUS), volume)

func mute_music(muted: bool) -> void:
	var volume: float = 0.0 if muted else music_volume
	AudioServer.set_bus_volume(AudioServer.get_bus_index(MUSIC_BUS), volume)

func mute_sfx(muted: bool) -> void:
	var volume: float = 0.0 if muted else sfx_volume
	AudioServer.set_bus_volume(AudioServer.get_bus_index(SFX_BUS), volume)